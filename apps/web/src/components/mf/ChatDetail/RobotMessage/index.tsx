...
const RobotMessage = forwardRef((props: IProps, ref) => {
    ...
    // 渲染不同类型的块
    const renderBlock = (block: { type: string, content: string, jsonNumber?: number }, index: number) => {
        switch (block.type) {
            case "content":
                return <div key={index} className={styles.content}>{block.content}</div>;
            case "title":
                return <div key={index} className={styles.title}>{block.content}</div>;
            case "json":
                try {
                    ...
                    return (
                        <div key={index} className={styles.json}>
                            ...
                            <div className={styles.tableDes}>
                                {
                                    getTableDes(jsonData)
                                }
                            </div>
                            <div className={styles.relatedColumns}>
                                {
                                    '相关字段：' + (jsonData.relatedColumns || [])

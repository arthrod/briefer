...
type API = {
  updateSettings: (workspaceId: string, data: WorkspaceEditFormValues) => Promise<ApiWorkspace>
  updateOnboarding: (workspaceId: string, onboardingStep: OnboardingStep) => Promise<void>
}
type UseWorkspaces = [{ data: ApiWorkspace[]; isLoading: boolean }, API]
export const useWorkspaces = (): UseWorkspaces => {
  const swrRes = useSWR<ApiWorkspace[]>(`${NEXT_PUBLIC_API_URL()}/v1/workspaces`, fetcher)

  const updateSettings = useCallback(
    async (workspaceId: string, data: WorkspaceEditFormValues) => {
      const res = await fetch(`${NEXT_PUBLIC_API_URL()}/v1/workspaces/${workspaceId}`, {
        credentials: 'include',
        method: 'PUT',
        headers: {
    ...
    async (workspaceId: string, onboardingStep: OnboardingStep) => {
      await swrRes.mutate(
        async () => {
          ...
          const workspace: ApiWorkspace = await res.json()
          return (swrRes.data ?? []).map((w) => (w.id === workspace.id ? workspace : w))
        },
        {
          revalidate: true,

...
/**
 * Initiates AI-assisted editing of a SQL query.
 *
 * This function retrieves workspace information and determines the data source type in order to
 * edit a SQL query using AI. For DuckDB data sources, it directly streams the editing process;
 * for regular databases, it retrieves the database URL and credentials before initiating the stream.
 *
 * @param workspaceId - The identifier of the workspace used to fetch associated secrets and settings.
 * @param datasourceId - The data source identifier object, which is either a database (with an id) or a DuckDB type.
 * @param source - The raw SQL query string to be edited.
 * @param instructions - The AI instructions used to guide the edit of the SQL query.
 * @param event - A callback function invoked with the assistant model ID (or null) for status updates.
 * @param onSuggestions - A callback function invoked with AI-generated suggestions during the editing process.
 * @returns A Promise that resolves to the result of the streamed SQL editing process.
 * @throws Error if the specified database data source is not found.
 *
 * @example
 * ```typescript
 * const editedQueryStream = await editWithAI(
 *   'workspace_123',
 *   { type: 'db', id: 'datasource_456' },
 *   'SELECT * FROM users',
 *   'Add LIMIT clause',
 *   (modelId) => console.log('Assistant model:', modelId),
